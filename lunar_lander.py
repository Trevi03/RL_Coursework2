...
## Train model
def LunarLanderTrainer(save_model=False,get_LC=False):
    ## Hyperparameters
    policy = "MlpPolicy"
    n_env = 16  # 16
    n_steps = 1024
    ...
    if get_LC:
        # Create folder dir
        log_dir = "LunarLander/lc_log/"
        os.makedirs(log_dir, exist_ok=True)

        env = make_vec_env("LunarLander-v3", n_envs=n_env, monitor_dir=log_dir) # Parallel environments
        ...
        env = make_vec_env("LunarLander-v3", n_envs=n_env) # Parallel environments
...
if __name__=="__main__":
    LunarLanderTrainer(save_model=True,get_LC=True)
    plot_results("LunarLander/lc_log/","LunarLander-v3 Learning Curve")
    # saveLanderGif(filename="lander_ppo.gif",trained_model="ppo_lunarlander")
